...
# use ifelse() to convert "Yes" and "No" values to 1 and 0, respectively
data$armed <- ifelse(data$armed == "Yes", 1, 0)

# use aggregate() to get the sum of the "armed" column by race
sum_armed_by_race <- aggregate(data$armed, by = list(data$race), sum)

# rename the columns in the result
colnames(sum_armed_by_race) <- c("race", "sum_armed")

# print the result
print(sum_armed_by_race)
...
renderPlot({
  ggplot(data_subset(), aes(x = input$age, y = input$filter_variable))+
    geom_boxplot()
